    ...
    export default {
        ...
        watch: {
            'searchData.email': function (value) {
                ...
                axios.get(`/users/match-by-email/${this.searchData.email}`)
                    .then(res => this.userMatches = res.data.items);
            }
        },
        methods: {
            ...
            removeUser: function (user) {
                axios.delete(`/todo-access-user-group/${this.$route.params.id}/delete/${user.group_id}`)
                    .then(res => {
                        this.loadUsers();
                    });
            },
